...
async def migros_category_post(req: CategoryRequest):
    ...
    if result.get("page_errors"):
        ...
        result["download_url"] = (
            "https://migros-gpt-action.vercel.app/api/migros/export"
            "?url=" + quote(req.url, safe="")
            + "&category_id=" + quote(str(result.get("category_id") or ""), safe="")
            + "&page_count=" + quote(str(result.get("page_count_reported") or 1), safe="")
        )
    return result
...
async def _migros_category_by_known_id(url: str, category_id: str, page_count: int):
    slug = category_slug(url)
    screen_url = f"https://www.migros.com.tr/rest/search/screens/{slug}"
    ...
    async with httpx.AsyncClient(follow_redirects=True) as client:
        ...
        hit_count = find_first_value(first, ["hitCount", "hit_count", "totalCount", "total_count"])
        all_products = [
            normalize_product(x, 1, screen_url)
            for x in extract_product_dicts(first)
        ]
        ...
        async def fetch_page(page):
            ...
            data = await get_json(client, api_url)
            return page, api_url, data

        if page_count > 1:
            results = await asyncio.gather(
                *(fetch_page(p) for p in range(2, page_count + 1)),
                return_exceptions=True
            )

            for page_no, item in zip(range(2, page_count + 1), results):
                if isinstance(item, Exception):
                    page_errors.append({"page": page_no, "error": str(item)})
                    continue

                page, api_url, data = item
                candidates = extract_product_dicts(data)
                if not candidates:
                    page_errors.append({
                        "page": page,
                        "error": "No product objects found"
                    })
                    continue

                for x in candidates:
                    all_products.append(normalize_product(x, page, api_url))
...
async def migros_export(url: str, category_id: str, page_count: int):
    if not category_id or page_count < 1:
        raise HTTPException(400, "category_id and page_count are required")

    result = await _migros_category_by_known_id(
        url=url,
        category_id=category_id,
...
@app.get("/api/migros/export-check")
async def export_check(url: str, category_id: str, page_count: int):
    return {
        "ok": True,
        "url": url,
        "category_id": category_id,
